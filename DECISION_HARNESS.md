...
Codex는 이 프로젝트의 사용자-facing 응답과 Decision Harness 기록을 한국어로 작성한다.

한국어 고정 대상:
- Decision Harness 안내 메시지
- Decision 문서
- ROADMAP, SUMMARY
- Q&A, troubleshooting, rollback 기록
- 사용자 답변 템플릿과 Pass 기준

예외:
- 코드 식별자, API 경로, 타입명, 필드명
- 상태값과 에러 코드
- 명령어와 파일 경로
- 외부 문서 또는 코드 원문 인용

문서 템플릿의 섹션명도 가능한 한 한국어를 사용한다. 이미 정착된 `Status`, `Implementation`, `Level` 같은 Decision 메타데이터 키는 유지할 수 있지만, 설명 본문은 한국어로 쓴다.

---

## 1. Question Intent Rule

사용자의 질문은 먼저 의도를 분류한다.

### Opinion-only 질문

사용자가 Codex의 의견, 이유, 필요성, 평가, 추천, 해석을 묻는 경우에는 실행하지 않는다.
...
응답 원칙:
- Codex의 의견과 근거를 답한다.
- 선택지의 trade-off를 설명한다.
- 필요하면 하네스 기준상 왜 해당 항목을 확인하는지 설명한다.
- 문서 수정, Decision 상태 변경, 구현, 테스트 실행, 다음 단계 진행은 하지 않는다.
- "이렇게 하겠습니다", "진행하겠습니다"처럼 실행을 전제하는 표현을 쓰지 않는다.

### Action 요청

사용자가 명시적으로 작업을 요청한 경우에만 실행한다.
...
Action 요청이 들어오면 해당 작업이 Level 1, Level 2, Level 3 중 어디에 해당하는지 판단하고 이 문서의 절차를 따른다.
...
12. 여전히 사용자 판단이 필요한 필수 Level 3 하위 결정이 남아 있으면, 현재 응답 안에서 다음 Decision을 즉시 제시한다.
13. 다음 Decision을 제시할 때는 `DECISIONS.md`에 `Status: Proposed`로 예약하고, 개별 Decision 문서를 만든 뒤 사용자에게 선택지와 답변 템플릿을 공유한다.
14. 필수 Level 3 결정이 모두 통과된 경우에만 다음 Decision을 제시하지 않고 Implementation Batch Snapshot으로 넘어간다.
15. 모든 필수 Level 3 결정이 통과된 뒤 Implementation Batch Snapshot을 기록한다.
16. 구현한다.
17. 구현 결과에 따라 각 Decision의 `Implementation: Completed / Failed / Rolled Back`을 갱신한다.
18. 해당 스프린트의 `SUMMARY.md`를 생성하거나 갱신한다.
19. 다음 스프린트가 있다면 다음 스프린트의 `ROADMAP.md`를 생성하거나 갱신한다.

### Pass 후 다음 분기 제시 규칙

Codex는 Level 3 Decision을 Pass 처리한 뒤, 남은 필수 Level 3 분기가 있는지 즉시 확인한다.

- 남은 필수 Level 3 분기가 있으면 같은 응답에서 다음 Decision을 예약하고 제시한다.
- 다음 분기가 여러 개라면 의존성상 가장 먼저 통과해야 하는 1개를 기본으로 제시한다.
- 서로 독립적이고 작게 분리된 경우에도 한 번에 최대 2개까지만 제시한다.
- 남은 필수 Level 3 분기가 없다면 다음 Decision을 만들지 않고 Implementation Batch Snapshot으로 넘어간다.
- 단순히 “Pass 처리 완료”만 말하고 멈추지 않는다.
...
## 6. Evaluation And Follow-up Questions
...
사용자에게 롤백 또는 재검토 조건 작성을 Pass 기준으로 요구하지 않는다. 롤백 계획, 재검토 조건, 재진입 조건은 Codex가 Decision 문서와 rollback 문서에 정리한다.
...
## 9. Sprint Completion Rule

스프린트 구현이 완료되면 Codex는 구현 완료 상태가 다음 작업으로 자연스럽게 이어지도록 완료 문서를 만든다.

완료 문서는 새로운 Decision을 자동 확정하기 위한 문서가 아니다. 구현 결과, 검증 상태, 다음 스프린트 후보 지도를 분리해 남기는 문서다.

### 9.1 Current Sprint Summary

현재 스프린트의 `docs/decisions/sprint-N/SUMMARY.md`를 생성하거나 갱신한다.
...
- 구현 결과
- 테스트와 검증 결과
- Level 1 또는 Level 2로 낮아진 후보
- 다음 스프린트로 넘기는 항목
- 알려진 제약이나 미검증 항목

### 9.2 Next Sprint Roadmap

다음 스프린트가 `docs/taejung/development-order.md`에 정의되어 있으면 `docs/decisions/sprint-(N+1)/ROADMAP.md`를 생성하거나 갱신한다.
...
- 다음 스프린트의 실제 Decision 문서는 자동 생성하지 않는다.
- 실제 사용자 선택이 필요한 후보만, 다음 스프린트를 진행할 때 `DECISIONS.md`에 ID를 예약한 뒤 개별 Decision 문서로 승격한다.
- 이전 스프린트의 완료가 다음 스프린트 후보를 자동 확정하지 않는다.
