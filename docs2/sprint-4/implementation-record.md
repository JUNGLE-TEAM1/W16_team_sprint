...
| 다이어그램에서 코드 읽는 법 | 전체 문법을 다 읽지 않아도 됩니다. 각 Mermaid 단계의 괄호 안에 적힌 파일/함수만 따라가면 됩니다. 예를 들어 `App.jsx loadPosts()`에서 요청이 시작되고, `posts.py list_posts`, `post_service.py list`, `post_repository.py list`, `schemas/post.py PostPage` 순서로 보면 됩니다. |
...
sequenceDiagram
    ...
    Browser->>Router: POST /api/v1/posts with tags (App.jsx createPost)
...
sequenceDiagram
    ...
    Browser->>Router: PATCH /api/v1/posts/{post_id} (App.jsx updatePost)
    Router->>Auth: session cookie 검증 (auth.py get_session_user)
    Auth-->>Router: current_user 반환
...
sequenceDiagram
    ...
    Browser->>Router: GET /api/v1/posts?q=&search_type=&tag=&page=&size= (App.jsx loadPosts)
    ...
    Service-->>Browser: PostPage 응답 -> setPosts/setPageMeta (App.jsx)
```
...
```mermaid
flowchart TD
    A["1. 사용자가 태그 버튼 클릭<br/>App.jsx filterByTag()"] --> B["2. React search.tag 변경<br/>App.jsx search state"]
    B --> C["3. GET /api/v1/posts?tag=fastapi&page=1&size=9<br/>App.jsx loadPosts()"]
...
```mermaid
flowchart TD
    A["1. 화면 진입<br/>App.jsx useEffect"] --> B["2. loadPosts(page=1, size=9)<br/>App.jsx loadPosts()"]
    A --> C["3. loadTags()<br/>App.jsx loadTags()"]
    B --> D["4. 게시글 카드 3열 그리드 표시<br/>App.jsx posts.map / styles .post-grid"]
    C --> E["5. 태그 버튼 표시<br/>App.jsx tags.map / styles .tag-filter"]
    F["6. 검색 폼 제출<br/>App.jsx submitSearch()"] --> G["7. q + search_type으로 목록 재조회<br/>App.jsx loadPosts()"]
    E --> H["8. 태그 버튼 클릭<br/>App.jsx filterByTag()"]
    H --> I["9. tag 단일 필터로 목록 재조회<br/>App.jsx loadPosts()"]
    J["10. 이전/다음 클릭<br/>App.jsx changePage()"] --> K["11. page 변경 후 목록 재조회<br/>App.jsx loadPosts()"]
    D --> L["12. 카드 클릭<br/>App.jsx selectPost()"]
...
현재 구현에서는 이 상태를 전부 `frontend/src/App.jsx` 안에서 React `useState`로 관리합니다.
...
```text
local state = App.jsx 컴포넌트 안에서만 관리하는 상태
```

현재 앱은 화면 구조가 크지 않기 때문에 `useState` 기반 local state로 충분합니다.
...
custom hook:
  App.jsx가 너무 길어질 때 loadPosts, submitSearch, filterByTag 같은 로직을 별도 함수 묶음으로 분리하는 방식이다.
...
이번 구현 = App.jsx 안에서 useState로 검색어/태그/페이지/게시글 목록을 관리
```
