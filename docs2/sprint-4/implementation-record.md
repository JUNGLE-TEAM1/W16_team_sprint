...
| 다이어그램에서 코드 읽는 법 | 전체 문법을 다 읽지 않아도 됩니다. 각 Mermaid 단계의 괄호 안에 적힌 파일/함수만 따라가면 됩니다. 현재 리팩터링 기준으로는 `usePostSearch.ts loadPosts()`에서 목록 요청이 시작되고, `posts.py list_posts`, `post_service.py list`, `post_repository.py list`, `schemas/post.py PostPage` 순서로 보면 됩니다. |
...
Sprint 4 구현 당시에는 검색, 태그, 페이징, 게시글 목록 상태가 `useBoardController.ts`에 있었습니다. 2026-06-16 hook 리팩터링 이후 현재 코드는 아래처럼 나뉘었습니다.

```text
frontend/src/hooks/usePostSearch.ts
  posts, tags, search, pageMeta
  loadPosts(), loadTags(), submitSearch(), filterByTag(), clearFilters(), changeSort(), changePage()

frontend/src/hooks/usePosts.ts
  postForm, editForm
  createPost(), updatePost(), selectPost()

frontend/src/hooks/useApiRequest.ts
  request()

frontend/src/hooks/useBoardController.ts
  usePostSearch/usePosts/useApiRequest를 조립하고 App.tsx에 board.* 형태로 반환
```

따라서 이 문서에서 `useBoardController.ts loadPosts/filterByTag/changePage`처럼 적힌 부분은 현재 코드 기준으로 `usePostSearch.ts`를 보면 됩니다.
...
sequenceDiagram
    ...
    Browser->>Router: POST /api/v1/posts with tags (usePosts.ts createPost)
...
sequenceDiagram
    ...
    Browser->>Router: PATCH /api/v1/posts/{post_id} (usePosts.ts updatePost)
    Router->>Auth: session cookie 검증 (auth.py get_session_user)
    Auth-->>Router: current_user 반환
...
sequenceDiagram
    ...
    Browser->>Router: GET /api/v1/posts?q=&search_type=&tag=&page=&size= (usePostSearch.ts loadPosts)
    ...
    Service-->>Browser: PostPage 응답 -> setPosts/setPageMeta (usePostSearch.ts)
```
...
```mermaid
flowchart TD
    A["1. 사용자가 태그 버튼 클릭<br/>TagFilter.tsx"] --> B["2. React search.tag 변경<br/>usePostSearch.ts filterByTag()"]
    B --> C["3. GET /api/v1/posts?tag=fastapi&page=1&size=9<br/>usePostSearch.ts loadPosts()"]
...
```mermaid
flowchart TD
    A["1. 화면 진입<br/>App.tsx"] --> B["2. loadPosts(page=1, size=9)<br/>usePostSearch.ts loadPosts()"]
    A --> C["3. loadTags()<br/>usePostSearch.ts loadTags()"]
    B --> D["4. 게시글 카드 3열 그리드 표시<br/>PostList.tsx / styles .post-grid"]
    C --> E["5. 태그 버튼 표시<br/>TagFilter.tsx / styles .tag-filter"]
    F["6. 검색 폼 제출<br/>HeroSearch.tsx"] --> G["7. q + search_type으로 목록 재조회<br/>usePostSearch.ts submitSearch() -> loadPosts()"]
    E --> H["8. 태그 버튼 클릭<br/>TagFilter.tsx"]
    H --> I["9. tag 단일 필터로 목록 재조회<br/>usePostSearch.ts filterByTag() -> loadPosts()"]
    J["10. 이전/다음 클릭<br/>PostList.tsx"] --> K["11. page 변경 후 목록 재조회<br/>usePostSearch.ts changePage() -> loadPosts()"]
    D --> L["12. 카드 클릭<br/>PostList.tsx -> usePosts.ts selectPost()"]
...
Sprint 4 구현 당시에는 이 상태를 대부분 `frontend/src/hooks/useBoardController.ts` 안에서 React `useState`로 관리했습니다.
현재 리팩터링 기준에서는 목록/검색/태그/페이징 상태는 `frontend/src/hooks/usePostSearch.ts`, 게시글 작성/수정 폼 상태는 `frontend/src/hooks/usePosts.ts`가 관리합니다.
`frontend/src/App.tsx`는 `useBoardController.ts`가 조립해서 돌려준 상태와 함수를 각 컴포넌트에 내려주며 화면을 조립합니다.
...
이번 Sprint의 상태 관리 방식은 여전히 `local state`입니다.

```text
local state = 기능별 hook 안에서 관리하고,
useBoardController.ts가 필요한 값과 함수를 모아 App.tsx를 통해 하위 컴포넌트로 전달하는 상태
```
...
custom hook:
  현재 적용된 방식이다.
  usePostSearch.ts가 loadPosts, submitSearch, filterByTag 같은 목록 탐색 로직을 관리한다.
  usePosts.ts가 게시글 작성/수정 폼과 CRUD 로직을 관리한다.
  useBoardController.ts는 이 hook들을 조립한다.
```
...
그래서 현재 코드는 서버 상태도 React hook에서 직접 관리합니다. 다만 리팩터링 이후에는 모든 서버 상태가 `useBoardController.ts`에 몰려 있지 않고, 기능별 hook으로 나뉘어 있습니다.

예를 들어 게시글 목록은 아래 흐름으로 관리합니다.

```text
1. 화면이 먼저 렌더링된다.
2. useEffect 또는 사용자 행동으로 `usePostSearch.ts`의 loadPosts()가 실행된다.
3. loadPosts()가 `useApiRequest.ts`의 request()를 통해 FastAPI에 GET /api/v1/posts 요청을 보낸다.
...
```text
클라이언트 상태와 서버 상태를 모두 React hook에서 useState/useEffect/fetch로 직접 관리하고 있다.
다만 현재는 useBoardController.ts 한 파일이 아니라 usePostSearch/usePosts/useComments/useAuth/useRelatedPosts로 책임을 나눴다.
```
...
이번 구현 = 기능별 custom hook 안에서 useState로 검색어/태그/페이지/게시글 목록을 관리하고, useBoardController.ts는 조립, App.tsx는 화면 조립을 담당
```
