...
Decision Harness의 사용자-facing 응답과 기록 문서는 한국어로 작성한다.
...
이미 정착된 `Status`, `Implementation`, `Level` 같은 Decision 메타데이터 키는 유지할 수 있지만, 설명 본문과 선택지 안내는 한국어로 쓴다.

---

## 1. 질문 의도 분류

Decision Harness는 모든 사용자 질문을 실행 요청으로 취급하지 않는다.

먼저 사용자의 말이 다음 중 어디에 해당하는지 구분한다.

### Opinion-only

사용자가 Codex의 의견, 이유, 필요성, 평가, 추천, 해석을 묻는 경우다.
...
- 의견과 근거를 설명한다.
- trade-off를 설명한다.
- 하네스 기준상 왜 묻는 항목인지 설명한다.
- 문서 수정, Decision 상태 변경, 구현, 테스트 실행, 다음 단계 진행은 하지 않는다.
- 실행을 전제한 "이렇게 하겠습니다" 식의 응답을 하지 않는다.

### Action

사용자가 명시적으로 작업을 요청한 경우다.
...
Action 요청일 때만 Decision 문서 변경, 상태 갱신, 구현, 테스트 실행 같은 작업을 진행한다.
...
```mermaid
flowchart TD
    Start["사용자 요청 접수"] --> Intent{"질문 의도"}
    Intent -->|Opinion-only| Opinion["의견과 근거만 답변"]
    Intent -->|Action| ReadRules["작업 규칙과 스프린트 기준 확인"]
    ...
    MoreL3 -->|Yes| NextDecision["같은 응답에서 다음 Decision 예약 및 제시"]
    NextDecision --> PickFirst
    ...
    Complete --> Summary["현재 스프린트 SUMMARY.md 생성/갱신"]
    Summary --> NextRoadmap["다음 스프린트 ROADMAP.md 생성/갱신"]
```

---

## 2-1. Pass 후 다음 분기 제시 규칙
...
- 남은 필수 Level 3 분기가 없으면 다음 Decision을 만들지 않고 Implementation Batch Snapshot으로 넘어간다.
...
스프린트 구현이 완료되면 Codex는 다음 문서를 생성하거나 갱신한다.

현재 스프린트:
- `docs/decisions/sprint-N/SUMMARY.md`

다음 스프린트:
- `docs/decisions/sprint-(N+1)/ROADMAP.md`
...
다음 스프린트 `ROADMAP.md`에는 다음을 기록한다.
...
- 다음 스프린트 진행 시 실제 사용자 선택이 필요한 후보만 `DECISIONS.md`에 ID를 예약한 뒤 개별 Decision 문서로 승격한다.
