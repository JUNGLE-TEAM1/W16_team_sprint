...
경기도 수원시 청년지원사업 OpenAPI 기반으로 사용자의 상황에 맞는 청년정책, 월세/취업/창업/교육 지원 카드를 찾아주는 AI 상담 보드입니다.

기존 게시판 구조를 유지하되 `Post`는 `data-bot`이 적재한 수원시 청년지원사업 카드 중심으로 재해석합니다. 사용자가 입력한 현재 상황은 저장하지 않는 일회성 RAG/Agent 요청으로만 처리합니다. `Tag`는 수원시, 청년, 월세, 취업, 창업, 교육 같은 조건 필터가 되고, RAG는 사용자의 현재 상황과 실제 수원시 API 카드 데이터를 매칭합니다.

## 현재 컨셉

- 서비스명: AI 수원 청년정책 매칭 보드
- 사용자 입력: "수원시 거주 24세 청년, 월세 60만 원. 받을 수 있는 지원 있나요?" 같은 일회성 매칭 요청
- AI 결과: 수원시 청년월세, 취업/창업/교육 지원 후보, 신청 체크리스트, 문의처
- 데이터: `data-bot`이 경기도 수원시_청년지원사업 OpenAPI에서 가져온 실제 정책 카드
- 확장: 동일 API의 연도별 endpoint 또는 공공데이터포털 API 행을 `Post` 카드로 변환 가능

## DB 목표 구조와 v1 매핑

장기 목표는 역할별 테이블을 분리하는 것입니다.

- `support_cards`: 수원시 청년지원사업 API에서 가져온 정책 카드
- `consultation_cases`: 사용자가 입력한 상담 상황. v1에서는 저장하지 않고, 이후 필요할 때 별도 테이블로 분리
- `data_sources`: 경기도 수원시_청년지원사업 OpenAPI와 수원청년포털 URL 출처
- `tags`: 수원시, 경기도, 청년, 청년정책, 월세, 취업, 창업, 교육 같은 분류
...
curl -X POST http://127.0.0.1:8000/api/v1/posts \
  ...
  -d '{"title":"수원시 청년 월세 상담","content":"수원시 거주 24세 청년이고 월세 60만 원을 내고 있습니다.","tag_names":["수원시","청년","월세"]}'
```
...
RAG는 수원시 청년지원사업 API에서 적재한 지원 카드와 MCP/외부 참고자료를 함께 사용합니다.
...
SUWON_YOUTH_POLICY_API_KEY=공공데이터포털_일반_인증키_Encoding
SUWON_YOUTH_POLICY_API_BASE_URL=https://api.odcloud.kr/api
SUWON_YOUTH_POLICY_API_PER_PAGE=100
...
참고자료 수집은 `REFERENCE_FETCH_ENABLED=true`일 때 동작합니다. RAG 서비스가 stdio MCP client로 `backend.app.mcp.reference_server`를 실행하고, MCP tool이 참고자료를 반환합니다. `POST /api/v1/rag/assist` 요청에 `reference_urls`를 넣으면 해당 URL만 참고자료로 가져오고, URL이 없을 때만 `REFERENCE_API_URL` 외부 API와 경기도 수원시 청년지원사업 OpenAPI, 수원청년포털, 수원시청 fallback을 사용합니다.
