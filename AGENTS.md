...
이 프로젝트의 Codex 응답, Decision Harness 안내, Decision 문서, ROADMAP, SUMMARY, Q&A, troubleshooting 기록은 기본적으로 한국어로 작성한다.

코드 식별자, API 경로, 타입명, 필드명, 상태값, 에러 코드, 명령어, 원문 인용처럼 기술적으로 그대로 유지해야 하는 표현만 영어를 허용한다.
...
## Question Intent Rule

사용자의 질문이 실행 요청인지 의견 요청인지 먼저 구분한다.

다음처럼 사용자가 Codex의 생각, 이유, 필요성, 평가, 추천, 해석을 묻는 경우에는 실행하지 않는다.
...
이 경우 Codex는 의견, 근거, trade-off, 필요한 경우 대안만 답한다. 문서 수정, Decision 상태 변경, 구현, 테스트 실행, 다음 단계 진행을 자동으로 하지 않는다.

실행하려면 사용자가 "추가해줘", "수정해줘", "진행해줘", "구현해줘", "기록해줘"처럼 명시적으로 작업을 요청해야 한다.
...
13. 여전히 사용자 판단이 필요한 필수 Level 3 후속 분기가 있으면, 같은 응답에서 다음 Decision을 즉시 예약하고 제시한다.
14. 남은 필수 Level 3 분기가 없을 때만 구현 전에 `Implementation Batch Snapshot`을 기록한다.
15. 구현한다.
16. 구현 결과에 따라 `Implementation: Completed / Failed / Rolled Back`으로 갱신한다.
17. 구현이 완료되면 현재 스프린트의 `SUMMARY.md`를 생성하거나 갱신한다.
18. 다음 스프린트가 정의되어 있으면 다음 스프린트의 `ROADMAP.md`를 생성하거나 갱신한다.
...
- 현재 스프린트의 `docs/decisions/sprint-N/SUMMARY.md`
- 다음 스프린트가 `docs/taejung/development-order.md`에 정의되어 있다면 `docs/decisions/sprint-(N+1)/ROADMAP.md`

`SUMMARY.md`에는 완료 기준, 완료된 Decision, 구현 결과, 테스트 결과, 낮아진 후보, 다음 스프린트로 넘길 항목을 기록한다.

다음 스프린트 `ROADMAP.md`에는 다음 스프린트 목표, 완료 기준, 이전 스프린트에서 이미 구현된 기반, 후보 분기 지도, 예상 Level, 의존성, 예상 첫 Decision을 기록한다.

주의:

- 다음 스프린트의 실제 Decision 문서는 자동 생성하지 않는다.
- 실제 사용자 선택이 필요한 후보만 다음 스프린트 진행 시 `DECISIONS.md`에 ID를 예약한 뒤 개별 Decision 문서로 승격한다.
...
Decision Harness 기록에는 롤백 계획과 재검토 조건을 포함한다. 단, 이를 사용자 답변 필수 항목으로 요구하지 않는다.
